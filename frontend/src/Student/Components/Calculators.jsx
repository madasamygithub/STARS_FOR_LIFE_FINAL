...
function Calculators() {
  ...
  const handleAddMore = () => {
    ...
    newRows.push({ id: newId, title: mode === "CGPA" ? `Semester ${newId}` : `Subject ${newId}`, grade: "", credits: "", gpa: "" });
    setRows(newRows);
  };
  ...
  const calculateCGPA = () => {
    ...
    return totalOverallCredits === 0 ? 0 : totalOverallWeightedGPAs / totalOverallCredits;
  };

  const handleCalculate = () => {
    const result = mode === "CGPA" ? calculateCGPA().toFixed(2) : calculateGPA().toFixed(2);
      ...
      <div className="calculators-container">
        ...
        <div className="filters">
          ...
          <div className="rightSide">
            <div className="toggle">
              <p>CGPA</p>
              <div className="toggle-btn" onClick={handleToggle} ref={toggleButtonRef}>
                <div className={`btn ${mode === "GPA" ? "active" : ""}`}></div>
              </div>
              <p>GPA</p>
            </div>
          </div>
        ...
        <div className="calculator">
          <table className="calculator-table">
            <thead>
              <tr>
                <th ref={calculatorModeRef}>{mode === "CGPA" ? "CGPA Calculator" : "GPA Calculator"}</th>
                ...
                <tr key={row.id}>
                  ...
                  <td>
                    {mode === "CGPA" ? (
                      <input
                        ...
                        onChange={(e) => {
                          const newRows = [...rows];
                          newRows.find((r) => r.id === row.id).gpa = e.target.value;
                          setRows(newRows);
                        }}
                      />
                    ) : (
                      <select
                        ...
                        onChange={(e) => {
                          const newRows = [...rows];
                          newRows.find((r) => r.id === row.id).grade = e.target.value;
                          setRows(newRows);
                        }}
                      >
                  ...
                  <td>
                    <input
                      ...
                      onChange={(e) => {
                        const newRows = [...rows];
                        newRows.find((r) => r.id === row.id).credits = e.target.value;
                        setRows(newRows);
                      }}
                    />
                  </td>
          ...
          <button className="add-more-btn" ref={addButtonRef} onClick={handleAddMore}>
            {mode === "CGPA" ? "Add More Semesters" : "Add More Subjects"}
          </button>
          <button className="calculate-btn" ref={calculateButtonRef} onClick={handleCalculate}>
